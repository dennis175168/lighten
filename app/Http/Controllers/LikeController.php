...
class LikeController extends Controller
{
    //
     public function like(){
        ...
        $check = Like::where([['user_id', '=', $user_id],['fixed_reports_id', '=', $fixed_reports_id]])->first();//有沒有按過like
        $check1 = Dislike::where([['user_id', '=', $user_id],['fixed_reports_id', '=', $fixed_reports_id]])->first();//有沒有按過dislike
        

        if($check1==null)//若是沒按過dislike->可以按like
        {
            if($check==null)//若是沒按過like
            {   
                Like::insert([
